...
typedef struct {
  ...
  // Array support
  int is_array;
  int array_size; // total elements (rows * cols for 2D)
  int array_rows; // rows for 2D (0 for 1D)
  int array_cols; // cols for 2D (0 for 1D)
  ValType array_elem_type;
  void *array_data; // Points to double* (contiguous, row-major for 2D)
} Var;
...
int find_block_end(int i) {
  ...
  i++;
  while (i < line_count && program[i].indent > base)
    i++;
  return i;
}
...
double eval_term(const char *s) {
  ...
  if (!strncmp(str, "len", 3) && str[3] == '(') {
    char *rp = strrchr(str, ')');
    if (!rp)
      error("unmatched parentheses in len()", NULL);
    ...
    // Check for array indexing in argument: len(a[i]) for 2D column count
    char *bracket = strchr(arg, '[');
    if (!bracket) {
      // Simple variable: len(a)
      Var v;
      if (!get_var(arg, &v)) {
      ...
      if (v.is_array) {
        // For 2D arrays, return number of rows
        if (v.array_cols > 0) {
          return (double)v.array_rows;
        }
        // For 1D arrays, return element count
      ...
      // len(a[i]) - for 2D arrays, return column count
      char varname[64];
      strncpy(varname, arg, bracket - arg);
      varname[bracket - arg] = 0;
      trim(varname);

      Var v;
      if (!get_var(varname, &v)) {
        error("len() undefined variable", varname);
      }

      if (v.is_array && v.array_cols > 0) {
        // 2D array: len(a[i]) returns column count
        // We don't actually need to evaluate the index since all rows have same
        // column count
        return (double)v.array_cols;
      } else {
        error("len() with index only valid for 2D arrays", varname);
      }
    }
  }
  ...
  if (bracket && !strchr(str, '(')) { // Not a function call
    ...
    if (!bracket_end)
      error("unmatched brackets in indexing", NULL);
    ...
    if (idx != (int)idx)
      error("array/string index must be integer", NULL);
    int index = (int)idx;
    ...
    if (v.is_array) {
      // Check for 2D indexing: a[i][j]
      if (v.array_cols > 0) {
        // This is a 2D array - need second index
        char *second_bracket = bracket_end + 1;
        while (*second_bracket == ' ' || *second_bracket == '\t')
          second_bracket++;

        if (*second_bracket != '[') {
          error("2D array requires two indices: a[row][col]", varname);
        }

        char *second_bracket_end = strchr(second_bracket, ']');
        if (!second_bracket_end)
          error("unmatched brackets in 2D indexing", varname);

        char col_expr[MAX_LINE];
        strncpy(col_expr, second_bracket + 1,
                second_bracket_end - second_bracket - 1);
        col_expr[second_bracket_end - second_bracket - 1] = 0;
        trim(col_expr);

        double col_idx = eval_condition(col_expr);
        if (col_idx != (int)col_idx)
          error("array column index must be integer", varname);
        int col = (int)col_idx;
        int row = index; // First index is row

        // Bounds checking
        if (row < 0 || row >= v.array_rows) {
          error("2D array row index out of bounds", varname);
        }
        if (col < 0 || col >= v.array_cols) {
          error("2D array column index out of bounds", varname);
        }

        // Row-major access: arr[row * cols + col]
        double *arr = (double *)v.array_data;
        return arr[row * v.array_cols + col];
      }
  ...
  if (lp) {
    ...
    if (fi != -1) {
      char *rp = strrchr(str, ')');
      if (!rp)
        error("unmatched parentheses", NULL);
  ...
  if (get_var(str, &v)) {
    if (v.is_array)
      error("cannot use array in numeric expression", str);
    if (v.type != V_NUM)
      error("not a number", str);
    return v.num;
  }
...
EvalResult eval_typed_expr(const char *s) {
  ...
  if (bracket && !strchr(str, '(') && !strchr(str, '+') && !strchr(str, '-') &&
    ...
    if (bracket_end) {
      ...
      double idx = eval_condition(index_expr);
      if (idx != (int)idx)
        error("string index must be integer", NULL);
      int index = (int)idx;
  ...
  if (!strchr(str, ' ') && !strchr(str, '\t') && !strchr(str, '(') &&
    ...
    if (get_var(str, &v)) {
      if (v.is_array)
        error("cannot use array in expression", str);
      if (v.type == V_STR) {
        result.type = V_STR;
...
void exec_for_loop(int i) {
  ...
  if (!sem1)
    error("for loop requires semicolons", NULL);
  *sem1 = 0;

  char *sem2 = strchr(sem1 + 1, ';');
  if (!sem2)
    error("for loop requires two semicolons", NULL);
  *sem2 = 0;
...
void exec_for_in_range_loop(int i) {
  ...
  if (!in_keyword)
    in_keyword = strstr(after_for, "\tin");
  if (!in_keyword)
    error("for-in loop requires 'in' keyword", NULL);
  ...
  if (!range_start)
    error("for-in requires range()", NULL);

  char *paren = strchr(range_start, '(');
  if (!paren)
    error("range requires parentheses", NULL);

  // Find matching close parenthesis (handle nested parens like range(len(a)))
  char *range_end = paren + 1;
  int paren_depth = 1;
  while (*range_end && paren_depth > 0) {
    if (*range_end == '(')
      paren_depth++;
    else if (*range_end == ')')
      paren_depth--;
  ...
  if (paren_depth != 0)
    error("unmatched parentheses in range()", NULL);
  range_end--; // Point to the closing paren
  ...
  while (*p) {
    if (*p == '(')
      ...
      if (arg_count >= 3)
        error("range() takes at most 3 arguments", NULL);
...
void exec_if_block(int i) {
  ...
  if (!colon)
    error("missing colon after if condition", NULL);
  *colon = 0;
  ...
  while (j < block_end && !executed) {
    ...
    if (program[j].indent == base_indent && !strncmp(s, "elif", 4)) {
      ...
      char *elif_cond = s + 4;
      while (*elif_cond == ' ' || *elif_cond == '\t')
        elif_cond++;

      char *elif_colon = strchr(elif_cond, ':');
      if (!elif_colon)
        error("missing colon after elif condition", NULL);
      *elif_colon = 0;
...
void exec_statement(char *t) {
  ...
  if (!strncmp(t, "exit", 4) && t[4] == '(') {
    char *rp = strrchr(t, ')');
    if (!rp)
      error("unmatched parentheses in exit()", NULL);
  ...
  if (!strncmp(t, "print", 5)) {
    ...
    if (t[5] == '(') {
      ...
      char *rp = strrchr(t, ')');
      if (!rp)
        error("unmatched parentheses in print()", NULL);
      ...
      while (*p) {
        ...
        while (*arg_trimmed == ' ' || *arg_trimmed == '\t')
          arg_trimmed++;
        char *end = arg_trimmed + strlen(arg_trimmed) - 1;
        while (end >= arg_trimmed && (*end == ' ' || *end == '\t'))
          *end-- = 0;
        ...
        if (bracket && !strchr(arg_trimmed, '(') && *arg_trimmed != '"') {
          ...
          while (*vn == ' ' || *vn == '\t')
            vn++;
          if (vn != varname)
            memmove(varname, vn, strlen(vn) + 1);
          char *vn_end = varname + strlen(varname) - 1;
          while (vn_end >= varname && (*vn_end == ' ' || *vn_end == '\t'))
            *vn_end-- = 0;
          ...
          if (bracket_end) {
            ...
            double idx = eval_condition(index_expr);
            if (idx != (int)idx)
              error("string/array index must be integer", NULL);
            int index = (int)idx;
            ...
            if (get_var(varname, &v)) {
              if (v.type == V_STR) {
              ...
              } else if (v.is_array && v.array_cols > 0) {
                // 2D array: need second index
                char *second_bracket = bracket_end + 1;
                while (*second_bracket == ' ' || *second_bracket == '\t')
                  second_bracket++;
                ...
                double *arr = (double *)v.array_data;
                printf("%g", arr[row * v.array_cols + col]);
        ...
        if (*arg_trimmed == '"' &&
            arg_trimmed[strlen(arg_trimmed) - 1] == '"') {
  ...
  if (!strncmp(decl_start, "int", 3) &&
      (decl_start[3] == ' ' || decl_start[3] == '\t')) {
    is_typed = 1;
    declared_type = V_NUM;
    decl_start = trim(decl_start + 3);
  } else if (!strncmp(decl_start, "double", 6) &&
             (decl_start[6] == ' ' || decl_start[6] == '\t')) {
    is_typed = 1;
    declared_type = V_NUM;
    decl_start = trim(decl_start + 6);
  } else if (!strncmp(decl_start, "string", 6) &&
             (decl_start[6] == ' ' || decl_start[6] == '\t')) {
    is_typed = 1;
    declared_type = V_STR;
    decl_start = trim(decl_start + 6);
  ...
  // Check for array declaration: type name[size] or type name[rows][cols]
  char *array_bracket = strchr(decl_start, '[');
  if (array_bracket && is_typed) {
    char *array_bracket_end = strchr(array_bracket, ']');
    if (!array_bracket_end)
      error("unmatched brackets in array declaration", NULL);
    ...
    while (*vn_start == ' ' || *vn_start == '\t')
      vn_start++;
    if (vn_start != varname)
      memmove(varname, vn_start, strlen(vn_start) + 1);
    char *vn_end = varname + strlen(varname) - 1;
    while (vn_end >= varname && (*vn_end == ' ' || *vn_end == '\t'))
      *vn_end-- = 0;

    char size_expr[MAX_LINE];
    strncpy(size_expr, array_bracket + 1,
            array_bracket_end - array_bracket - 1);
    size_expr[array_bracket_end - array_bracket - 1] = 0;

    // Trim size_expr properly
    char *se_start = size_expr;
    while (*se_start == ' ' || *se_start == '\t')
      se_start++;
    if (se_start != size_expr)
      memmove(size_expr, se_start, strlen(se_start) + 1);
    char *se_end = size_expr + strlen(size_expr) - 1;
    while (se_end >= size_expr && (*se_end == ' ' || *se_end == '\t'))
      *se_end-- = 0;
    ...
    int dim1 = (int)size_val;

    // Check for 2D array: int a[rows][cols]
    char *second_bracket = array_bracket_end + 1;
    while (*second_bracket == ' ' || *second_bracket == '\t')
      second_bracket++;
    ...
    if (*second_bracket == '[') {
      ...
      if (declared_type != V_NUM) {
        error("2D arrays only support numeric types", varname);
      }

      char *second_bracket_end = strchr(second_bracket, ']');
      if (!second_bracket_end)
        error("unmatched brackets in 2D array declaration", varname);

      char cols_expr[MAX_LINE];
      strncpy(cols_expr, second_bracket + 1,
              second_bracket_end - second_bracket - 1);
      cols_expr[second_bracket_end - second_bracket - 1] = 0;

      // Trim cols_expr
      char *ce_start = cols_expr;
      while (*ce_start == ' ' || *ce_start == '\t')
        ce_start++;
      if (ce_start != cols_expr)
        memmove(cols_expr, ce_start, strlen(ce_start) + 1);
      char *ce_end = cols_expr + strlen(cols_expr) - 1;
      while (ce_end >= cols_expr && (*ce_end == ' ' || *ce_end == '\t'))
        *ce_end-- = 0;

      double cols_val = eval_condition(cols_expr);
      if (cols_val != (int)cols_val || cols_val <= 0) {
        error("array column size must be positive integer", varname);
      }
      ...
      if (total > MAX_ARRAY_SIZE) {
        error("2D array size too large", varname);
      }
      ...
      v.array_rows = rows;
      v.array_cols = cols;
      v.array_elem_type = V_NUM;
      v.is_const = is_const;
      v.explicit = is_typed;
      v.type = V_ARRAY;

      // Allocate contiguous memory (row-major)
      double *arr = (double *)calloc(total, sizeof(double));
      if (!arr)
        error("memory allocation failed for 2D array", varname);

      // Check for initialization
      char *eq = strchr(second_bracket_end, '=');
      if (eq && eq[1] != '=') {
        char *init_start = trim(eq + 1);

        if (*init_start != '[') {
          error("2D array initialization requires [[...], [...]] syntax",
                varname);
        }

        // Find matching outer bracket
        int depth = 0;
        char *init_end = init_start;
        while (*init_end) {
          if (*init_end == '[')
            depth++;
          else if (*init_end == ']') {
        ...
        if (depth != 0)
          error("unmatched brackets in 2D array initialization", varname);

        // Parse rows: [[1,2,3], [4,5,6], ...]
        char *p = init_start + 1; // Skip outer [
        int row_idx = 0;
        ...
        while (*p && p < init_end) {
          ...
          if (*p != '[') {
            error("2D array row must start with [", varname);
          }

          if (row_idx >= rows) {
            error("too many rows in 2D array initialization", varname);
          }
          ...
          if (!row_end)
            error("unmatched brackets in row initialization", varname);

          // Parse elements in this row
          char *elem_p = row_start;
          int col_idx = 0;

          while (elem_p < row_end) {
            // Skip whitespace
            while (elem_p < row_end && (*elem_p == ' ' || *elem_p == '\t'))
              elem_p++;
            if (elem_p >= row_end)
              break;

            // Find end of element
            char *elem_start = elem_p;
            int elem_depth = 0;
            while (elem_p < row_end && !(*elem_p == ',' && elem_depth == 0)) {
              if (*elem_p == '(')
                elem_depth++;
              else if (*elem_p == ')')
                elem_depth--;
            ...
            char elem[MAX_LINE];
            strncpy(elem, elem_start, elem_p - elem_start);
            elem[elem_p - elem_start] = 0;
            ...
            while (e_end >= e && (*e_end == ' ' || *e_end == '\t'))
              *e_end-- = 0;

            if (*e != '\0') {
              if (col_idx >= cols) {
                error("too many columns in row initialization", varname);
              }
              // Row-major: arr[row * cols + col]
              arr[row_idx * cols + col_idx] = eval_condition(e);
              col_idx++;
            }
          ...
          // Missing columns are already zero-filled by calloc

          row_idx++;
          p = row_end + 1;
        }

        // Missing rows are already zero-filled by calloc
      }
    ...
    v.array_rows = 0; // 0 = 1D array indicator
    v.array_cols = 0;
    ...
    if (eq && eq[1] != '=') {
      ...
      if (!init_end)
        error("unmatched brackets in array initialization", varname);
  ...
  // Handle array element assignment: arr[idx] = value or arr[i][j] = value
  // (check BEFORE regular assignment!)
  if (!is_typed) {
    ...
    if (bracket) {
      ...
      if (eq && eq[1] != '=' && eq[-1] != '=' && eq[-1] != '!' &&
        ...
        while (*ie_start == ' ' || *ie_start == '\t')
          ie_start++;
        if (ie_start != index_expr)
          memmove(index_expr, ie_start, strlen(ie_start) + 1);
        char *ie_end = index_expr + strlen(index_expr) - 1;
        while (ie_end >= index_expr && (*ie_end == ' ' || *ie_end == '\t'))
          *ie_end-- = 0;

        double idx = eval_condition(index_expr);
        if (idx != (int)idx)
          error("array index must be integer", NULL);
        int index = (int)idx;
        ...
        // Check for 2D array assignment: a[i][j] = value
        if (v.array_cols > 0) {
          // This is a 2D array
          char *second_bracket = bracket_end + 1;
          while (*second_bracket == ' ' || *second_bracket == '\t')
          ...
          if (*second_bracket != '[') {
            error("2D array requires two indices: a[row][col] = value",
                  varname);
          }
          ...
          // Trim col_expr
          char *ce_start = col_expr;
          while (*ce_start == ' ' || *ce_start == '\t')
            ce_start++;
          if (ce_start != col_expr)
            memmove(col_expr, ce_start, strlen(ce_start) + 1);
          char *ce_end = col_expr + strlen(col_expr) - 1;
          while (ce_end >= col_expr && (*ce_end == ' ' || *ce_end == '\t'))
            *ce_end-- = 0;

          double col_val = eval_condition(col_expr);
          if (col_val != (int)col_val)
            error("array column index must be integer", varname);
          int col = (int)col_val;
          int row = index; // First index is row
          ...
          // Find RHS (after second bracket's ])
          char *rhs = second_bracket_end + 1;
          while (*rhs == ' ' || *rhs == '\t')
            rhs++;
          if (*rhs != '=')
            error("expected '=' in 2D array assignment", varname);
          rhs++;
          while (*rhs == ' ' || *rhs == '\t')
            rhs++;
  ...
  for (int i = 0; i < line_count; i++) {
    ...
    if (!strncmp(t, "def", 3)) {
      ...
      *q = 0;
      ...
      char *tok = strtok(p + 1, ",");
      while (tok)
        strcpy(f->params[f->param_count++], trim(tok)), tok = strtok(NULL, ",");
